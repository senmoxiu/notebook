...
import {get,put} from "@/request/api.ts";
import {useUserStore} from "@/store/user.ts";
import {useStoreEvent} from "@/store/event.ts";

const router = useRouter()
const useUser = useUserStore()
const useEvents = useStoreEvent()
const {eId,cId,title,status,level,content,startDate,endDate} = storeToRefs(useEvents)
const date1 = ref(['', ''])
date1.value = [startDate.value, endDate.value]
const EventsForm = reactive({
  eId: eId,
  title: title,
  cId: cId,
  status: status,
...
const onSubmit = () => {
  EventsForm.startDate = date1.value[0]
  EventsForm.endDate = date1.value[1]
  console.log(EventsForm)
  put('/events', EventsForm)
      .then((res:any) => {
        console.log(res)
        router.push('/Events')
      })
    ...
    <el-form :model="EventsForm" label-width="80px" label-position="top" id="form">
      ...
      <el-form-item label="状态">
        <el-radio-group v-model="EventsForm.status">
          ...
          <el-radio-button :value="2">作废</el-radio-button>
        </el-radio-group>
      </el-form-item>
      <el-form-item  label="级别">
        <el-radio-group v-model="EventsForm.level">
          <el-radio-button :value="0">重要</el-radio-button>
